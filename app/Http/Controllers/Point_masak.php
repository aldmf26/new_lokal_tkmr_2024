...
class Point_masak extends Controller
{
    ...
    public function point_export_server(Request $r)
    {
        ...
        $sheet
            ...
            ->setCellValue('E1', 'POINT MASAK')
            ->setCellValue('F1', 'KOM POINT MASAK')
            ->setCellValue('G1', 'NON POINT MASAK')
            ->setCellValue('H1', 'KOM NON POINT MASAK')
            ->setCellValue('J1', 'Org P ')
            ->setCellValue('K1', $jumlah_orang->jumlah)
            ->setCellValue('J2', 'Org R ')
            ->setCellValue('K2', $orang)
            ->setCellValue('J4', 'Service charge P ')
            ->setCellValue('K4', ($service_charge / 7) * $persen->jumlah_persen)
            ->setCellValue('J5', 'Service charge R')
            ->setCellValue('K5', $kom);
        $kolom = 2;
        ...
        foreach ($masak as $k) {
            $sheet->setCellValue('A' . $kolom, $i++);
            $sheet->setCellValue('B' . $kolom, $k->nama);
            $sheet->setCellValue('C' . $kolom, $k->rp_m);
            $gaji = ($k->rp_m * $k->qty_m) + ($k->rp_e * $k->qty_e) + ($k->rp_sp * $k->qty_sp);
            $sheet->setCellValue('D' . $kolom, $gaji);
            $kom1 =  round(($k->point_berhasil / $point) * $kom, 0);
            $sheet->setCellValue('E' . $kolom, $k->point_berhasil);
            $sheet->setCellValue('F' . $kolom, $kom1);
            $sheet->setCellValue('G' . $kolom, $k->point_gagal);
            $kom3 =  round(($k->point_gagal / $point) * $kom);
            $sheet->setCellValue('H' . $kolom, $kom3);
            $kolom++;
        }
